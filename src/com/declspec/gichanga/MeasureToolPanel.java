...
public class MeasureToolPanel extends JPanel {
	...
	private JCheckBox modeS = new JCheckBox("Track Aircraft on SBS");
	...
	private void makePanel(Dimension size) {
		...
		bt.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent actionEvent) {
				...
				measureTool.setMeasureShape(
						new SurfaceQuad(Position.fromDegrees(44, 7, 0), 100e3, 50e3, Angle.fromDegrees(30)));
			}
		});
		presetPanel.add(bt);
		...
		metricPanel
				.setBorder(new CompoundBorder(new TitledBorder("Metric"), BorderFactory.createEmptyBorder(5, 5, 5, 5)));
		...
		// Preset buttons
		JPanel trackingPanel = new JPanel(new GridLayout());
		trackingPanel.setBorder(BorderFactory.createEmptyBorder(4, 4, 4, 4));

		modeS.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent actionEvent) {
				WorldWindPanel.getWorldWindPanel().fireSBSTrackingEvent(modeS.isSelected());
			}
		});
		trackingPanel.add(modeS);
